...
| `PROBLEMA-MODO-LIGERO.md` | 🆘 Descripción detallada del fallo del modo ligero para escalar a otra IA | compartir la URL raw del archivo |
...
- **Navegación inmediata + estados (v1.5)**: cambiar el destino (⚙) dispara la navegación al instante (`lanzar_calentar(forzar=True)`) — antes no pasaba nada hasta la próxima pregunta. Estado de página explícito (`cargando`/`por-verificar`/`lista`): el espejo responde "cargando" SIN tocar Marionette mientras la SPA carga (adiós a la contención del lock que causaba `ocupado:true` permanente).
- **Diagnóstico (v1.4+)**: `/salud` (SIN clave, nunca se cuelga) reporta `firefox_headless`, `marionette` (conectado/ocupado/escuchando/NO responde), `destino` y `pagina`. `/debug?clave=...` nunca se cuelga: si hay trabajo en curso responde `{ocupado, parcial, pagina}` de inmediato.
- **Precarga (v1.3)**: el servidor navega a `destino` apenas arranca (`lanzar_calentar()`), porque antes la página quedaba en blanco hasta la primera pregunta y el espejo mostraba vacío. Además `destino_actual()` AUTOVALIDA: si lo guardado no contiene "notion" vuelve al default (sanaea destinos malos heredados).
...
- **Destino por defecto**: https://www.notion.so/chat — cambiable en el ⚙ de la mini-web o en `destino.txt`. **Solo URLs de Notion** (el usuario puso por error la URL del VNC y el headless quedó navegando al noVNC — no repetir). OJO: el destino debe ser un CHAT de Notion AI (con cuadro "Pregunta..."); si es una página-documento normal, el puente escribiría texto EN el documento.
...
- **Estado de la prueba (21 ago 2026 ~23:18Z)**: puente vivo (`firefox_headless:true`, `marionette:conectado`), pero la página nunca se volvió legible (espejo vacío, `/debug` = `ocupado:true` persistente con v1.4). Destino corregido a `…/Chat-pesado-de-prueba-3269d22d183d80b0a680dfa3210a7942` (el usuario primero apuntó al MISMO chat de esta conversación — riesgo de bucle). Hipótesis principales: ExecuteScript espera al hilo principal saturado por la SPA con CPU capada; posible detección `navigator.webdriver`; login; o destino sin composer. **Se creó `PROBLEMA-MODO-LIGERO.md` con el handoff completo para otra IA.** Pendiente: `/debug` completo tras v1.5.
...
- **Si falla**: `sh modo.sh pantalla` restaura lo de siempre; pedir salida completa del comando + JSON de `/salud` y `/debug?clave=...` y ajustar selectores/heurísticas.
