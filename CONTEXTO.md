...
- **Nuevo (21 ago 2026, `modo.sh` v1.3):** interruptor pantalla↔**texto ligero** (experimental) — ver sección «Modo ligero» abajo. Motivo: el lag venía del VIDEO del VNC, no de la RAM.
...
- **Precarga (v1.3)**: el servidor navega a `destino` apenas arranca (`lanzar_calentar()`), porque antes la página quedaba en blanco hasta la primera pregunta y el espejo mostraba "(la pagina aun no muestra texto)". Además `destino_actual()` AUTOVALIDA: si lo guardado no contiene "notion" vuelve al default (sanaea destinos malos heredados).
...
- **Estado de la prueba (21 ago 2026 ~22:50Z)**: mini-web OK, clave OK, mensajes llegan al puente, espejo mostraba vacío (corregido en v1.3 con precarga). Pendiente confirmar que Notion AI responda de punta a punta.
