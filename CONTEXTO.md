...
- **Nuevo (21 ago 2026, `modo.sh` v1.4):** interruptor pantalla↔**texto ligero** (experimental) — ver sección «Modo ligero» abajo. Motivo: el lag venía del VIDEO del VNC, no de la RAM.
...
- **Diagnóstico (v1.4)**: `/salud` (SIN clave, nunca se cuelga) reporta `firefox_headless` (pgrep), `marionette` (conectado/ocupado/escuchando/NO responde — sondeo del saludo del protocolo sin sesión) y `destino`. `/debug?clave=...` (v1.1) nunca se cuelga: si hay trabajo en curso responde `{ocupado, parcial}` de inmediato.
...
- **Destino por defecto**: https://www.notion.so/chat — cambiable en el ⚙ de la mini-web o en `destino.txt`. **Solo URLs de Notion** (el usuario puso por error la URL del VNC y el headless quedó navegando al noVNC — no repetir).
...
- **Estado de la prueba (21 ago 2026 ~22:58Z)**: mini-web OK, clave OK, mensajes llegan al puente; el espejo NO muestra texto de Notion aun con precarga → pendiente diagnóstico con `/salud` y `/debug` de v1.4 (sospechas: marionette no levantó en Firefox 154, login vencido en el perfil, o carga muy lenta con la CPU capada). Mi lector web no alcanza las rutas del sandbox desde aquí — el diagnóstico lo abre el USUARIO en su navegador y pega el JSON.
