#!/usr/bin/env python3
# server.py v1.9.3 - puente de texto <-> Notion AI (Firefox headless + Marionette)
...
# v1.9.1: JS_NOMBRE v2 — evidencia del pod: el switcher del sidebar es un
#   boton cuyo innerText es "<letra avatar>\n<Nombre del espacio>" (en
#   /controles salia como "E" porque alli se toma solo la 1a linea).
#   Multi-estrategia: clase clasica -> 2a linea del primer boton con letra
#   sola -> aria-label con espacio/workspace.
# v1.9.2: /creditos (GET) -> abre my-settings, pulsa la entrada "Tu asignacion
#   de creditos" (matcher por texto) y devuelve el texto de la pantalla. Si no
#   la encuentra, devuelve el menu REAL de ajustes para corregir con evidencia.
# v1.9.3: CORREGIDA con evidencia real: my-settings da 404 (pagina no
#   encontrada en el pod, 22 ago). La ruta oficial (Centro de ayuda Notion):
#   modal Configuracion -> pestaña Notion AI -> Uso. Ahora se navega la UI por
#   texto, SIN URLs inventadas; cada paso que falla devuelve los controles
#   reales visibles como evidencia.
# v1.8.2:
...
"""
JS_NOMBRE = """
// nombre del workspace. Evidencia del pod: el switcher del sidebar es un
// boton cuyo innerText tiene la letra del avatar en la 1a linea y el
// nombre del espacio en la 2a.
function nombreDe(txt){
  const lineas = (txt || '').split('\\n').map(s => s.trim()).filter(Boolean);
  if (!lineas.length) return '';
  if (lineas.length >= 2 && lineas[0].length <= 2) return lineas[1];
  return lineas[0];
}
const sw = document.querySelector('.notion-sidebar-switcher');
if (sw) { const n = nombreDe(sw.innerText); if (n) return n.slice(0, 40); }
const nav = document.querySelector('nav') || document.body;
const bs = [...nav.querySelectorAll('div[role="button"], button')].filter(vis);
for (const b of bs) {
  const lineas = (b.innerText || '').split('\\n').map(s => s.trim()).filter(Boolean);
  if (lineas.length >= 2 && lineas[0].length <= 2) return lineas[1].slice(0, 40);
}
const c = bs.find(x => /espacio|workspace|cuenta|account/i.test(x.getAttribute('aria-label') || ''));
if (c) return (c.getAttribute('aria-label') || '').trim().slice(0, 40);
return '';
"""
...
def entrar(correo):
    ...
    time.sleep(1)
    # via 1: Enter real dentro del campo -> envia el form nativo
    try:
...
def creditos():
    # Sin URLs inventadas: my-settings dio 404 en el pod (evidencia 22 ago).
    # Ruta oficial del Centro de ayuda: Configuracion -> Notion AI -> Uso.
    def menu_real():
        try:
            return [c.get("t") for c in json.loads(js(JS_CONTROLES) or "[]")][:40]
        except Exception:
            return []
    # 1. abrir el modal de Configuracion desde el sidebar
    r = js(JS_CLIC_T % json.dumps("configuración"))
    if r == "no-esta":
        r = js(JS_CLIC_T % json.dumps("configurac"))
    if r == "no-esta":
        r = js(JS_CLIC_T % json.dumps("settings"))
    if r == "no-esta":
        return {"encontrado": False, "paso": "abrir Configuracion",
                "menu": menu_real(), "texto": js(JS_TEXTO) or ""}
    time.sleep(3)
    # 2. pestaña Notion AI dentro del modal
    r2 = js(JS_CLIC_T % json.dumps("notion ai"))
    if r2 == "no-esta":
        return {"encontrado": False, "paso": "pestana Notion AI",
                "menu": menu_real(), "texto": js(JS_TEXTO) or ""}
    time.sleep(2)
    # 3. sub-entrada Uso / Usage / asignacion / creditos
    r3 = "no-esta"
    for q in ("asignaci", "uso", "usage", "crédito", "credito", "credit"):
        r3 = js(JS_CLIC_T % json.dumps(q))
        if r3 != "no-esta":
            break
    time.sleep(2.5)
    texto = js(JS_TEXTO) or ""
    cache["texto"] = texto
    cache["cuando"] = time.time()
    pagina["estado"] = "otra-pagina"
    pagina["detalle"] = "leyendo la asignacion de creditos"
    return {"encontrado": r3 != "no-esta", "pulsado": [r, r2, r3],
            "texto": texto, "menu": [] if r3 != "no-esta" else menu_real()}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/salud":
            ...
            self._json({"ok": True, "version": "1.9.3", "firefox_headless": ff,
