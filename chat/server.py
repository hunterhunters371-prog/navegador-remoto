#!/usr/bin/env python3
# server.py v1.9.2 - puente de texto <-> Notion AI (Firefox headless + Marionette)
...
def creditos():
    navegar(NOTION + "my-settings")
    pagina["estado"] = "otra-pagina"
    pagina["detalle"] = "leyendo la asignacion de creditos"
    time.sleep(4)
    r = "no-esta"
    for q in ("asignaci", "crédito", "credito", "credit"):
        r = js(JS_CLIC_T % json.dumps(q))
        if r != "no-esta":
            break
    time.sleep(2.5)
    ...
    if r == "no-esta":
        try:
            menu = [c.get("t") for c in json.loads(js(JS_CONTROLES) or "[]")][:40]
        except Exception:
            menu = []
        return {"encontrado": False, "menu": menu, "texto": texto}
    return {"encontrado": True, "pulsado": r, "texto": texto}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/salud":
            ...
            self._json({"ok": True, "version": "1.9.2", "firefox_headless": ff,
